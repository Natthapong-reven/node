...
cctest: all ## Runs the C++ tests using the built `cctest` executable.
...
build-js-native-api-tests: | $(NODE_EXE) test/js-native-api/.buildstamp ## Builds JS Native-API tests.
...
# Just goes to show that recursive make really is harmful...
# TODO(bnoordhuis) Force rebuild after gyp or node-gyp update.
build-node-api-tests: | $(NODE_EXE) test/node-api/.buildstamp ## Builds Node-API tests.
...
test-hash-seed: all ## Verifies the hash seed used by V8 for hashing is random.
...
.PHONY: test-doc-ci
test-doc-ci: doc-only ## Builds, lints, and verifies the docs for CI.
...
test-v8-intl: v8 ## Runs the v8 test suite, intl tests.
	export PATH="$(NO_BIN_OVERRIDE_PATH)" && \
...
test-v8-benchmarks: v8 ## Runs the v8 test suite, benchmarks.
	export PATH="$(NO_BIN_OVERRIDE_PATH)" && \
		$(PYTHON) deps/v8/tools/run-tests.py --gn --arch=$(V8_ARCH) \
				benchmarks \
				$(TAP_V8_BENCHMARKS)
...
.PHONY: doc
doc: $(NODE_EXE) doc-only ## Builds Node.js, and then builds the documentation with the new binary.
...
.PHONY: pkg
pkg: $(PKG) ## Builds the macOS installer for releases.
...
.PHONY: lint-md
lint-md: lint-js-doc | tools/.mdlintstamp ## Lints the markdown documents maintained by us in the codebase.
...
lint-js lint-js-doc: ## Lints the JavaScript code with eslint.
	@if [ "$(shell $(node_use_openssl))" != "true" ]; then \
...
.PHONY: lint-cpp
lint-cpp: tools/.cpplintstamp ## Lints the C++ code with cpplint.py and checkimports.py.
...
lint-addon-docs: tools/.doclintstamp ## Lints the addon documentation.
